...
ggheatmap <- function(dataMatrix, orderCol = T, orderRow = T, dendroLineSize = 0.5, 
    ...
    heat_plot <- ggplot2::ggplot(data_m, aes(x = variable, y = rowname, fill = value)) + 
        ggplot2::geom_tile() + ggplot2::theme_minimal() + ggplot2::theme(axis.line = element_line(size = 0), 
        text = element_text(size = fontSize)) + ggplot2::scale_y_discrete(position = "right") + 
        ggplot2::xlab("") + ggplot2::ylab("") + ggplot2::scale_fill_distiller(palette = colorPalette, 
        name = scaleName)
    ...
    if (orderRow) {
        ...
        dendro_row <- cowplot::axis_canvas(heat_plot, axis = "y", coord_flip = TRUE) + 
            ggplot2::geom_segment(data = segment(dendro_data_row), aes(y = -y, x = x, 
                xend = xend, yend = -yend), size = dendroLineSize) + ggplot2::coord_flip()
    ...
    if (orderCol) {
        ...
        dendro_col <- cowplot::axis_canvas(heat_plot, axis = "x") + ggplot2::geom_segment(data = segment(dendro_data_col), 
